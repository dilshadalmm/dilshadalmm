...
const aiSolveQueue = [];
const normalizedCache = new Map(); // key: hash(text+image), value: normalized result
...
function cacheKey(text, imageBase64) {
    return crypto.createHash('sha256').update(text + (imageBase64 || '')).digest('hex');
}
...
// ---- Normalize + Validate (handles text + image together, cached) ----
async function normalizeAndValidate(input) {
    const key = cacheKey(input.text || "", input.imageBase64 || "");
...
`
    ...
    if (input.imageBase64) {
        parts.push({ inlineData: { mimeType: "image/png", data: input.imageBase64.split(',')[1] } });
    }
    ...
    try {
        ...
        result = { normalized_question: input.text || "", validity: "VALID" };
    }

    normalizedCache.set(key, result);
    return result;
}
