...
const aiSolveQueue = []; // queue for AI solves
...
// ---- Normalize + Validate (handles OCR if image) ----
async function normalizeAndValidate(input) {
    const contentText = input.text || "";
    const imageData = input.imageBase64 ? `Image included.` : "";
    const combinedInput = `${contentText} ${imageData}`.trim();
...
`
    ...
    try {
        ...
        return { normalized_question: combinedInput, validity: "VALID" };
    }
}
...
async function processBatch(batch) {
    ...
    // Step 1: Normalize + Validate (handles text or image)
