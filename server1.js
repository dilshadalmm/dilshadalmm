...
const AI_BATCH_SIZE = 5;      // Number of simultaneous AI calls
const AI_BATCH_DELAY = 200;   // ms delay between batches
...
const aiSolveQueue = []; // separate queue for AI solves
...
async function runGeminiOCR(imageBase64) {
    ...
    const response = await ai.models.generateContent({
        ...
        contents: [{
            role: "user",
            parts: [
                { text: "Extract the question text from this image." },
...
// ---- Normalize + Validate ----
async function normalizeAndValidate(question) {
    const response = await ai.models.generateContent({
        model: "gemini-2.5-flash-lite",
        contents: [{
            role: "user",
...
Also classify if the question is VALID academic question.
...
`
    ...
    try {
        ...
        return { normalized_question: question, validity: "VALID" };
    }
}
...
async function processBatch(batch) {
    ...
    for (const item of batch) {
        ...
        const normalized = await normalizeAndValidate(text);
    ...
    const queries = validItems.map((item, i) => index.query({ vector: vectors[i], topK: 3, includeMetadata: true }));
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (bestMatch && bestMatch.score >= 0.85) {
            ...
            // Push to AI solve queue
            aiSolveQueue.push({ item, vector, question: texts[i] });
        }
    }
}
...
processAISolveQueue(); // start background AI worker
