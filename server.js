...
Given the following question text and possibly an image, rewrite the question clearly without changing meaning. If the image contains an educational diagram, describe it as well. Also classify if the question is a VALID academic question.
Return JSON:
{
  "normalized_question": "...",
...
async function normalizeAndValidate(text, imageBase64) {
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            console.error("Could not parse JSON from normalization response:", fullText);
            return { normalized_question: "", validity: "INVALID" };
        }
    } catch (err) {
        ...
        async function processNextBatch() {
            ...
            try {
                ...
                const upserts = batch.map((doc, idx) => ({
                    id: doc.id,
                    values: vectors[idx],
                    metadata: { text: texts[idx] }  // Note: old format, no solution
                }));
