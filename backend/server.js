...
async function updateMetadataCounts(className, subject, chapter, incrementBy) {
  ...
  await db.collection('courses').doc(courseId).set(
    { name: className, totalQuestions: admin.firestore.FieldValue.increment(incrementBy), updatedAt: admin.firestore.FieldValue.serverTimestamp() },
    { merge: true }
  );
  await db.collection('subjects').doc(subjectId).set(
    { class: className, subject: subject, totalQuestions: admin.firestore.FieldValue.increment(incrementBy), updatedAt: admin.firestore.FieldValue.serverTimestamp() },
    { merge: true }
  );
  await db.collection('chapters').doc(chapterId).set(
    { class: className, subject: subject, chapter: chapter, totalQuestions: admin.firestore.FieldValue.increment(incrementBy), updatedAt: admin.firestore.FieldValue.serverTimestamp() },
    { merge: true }
  );
}
...
async function loadMetadataCache() {
  ...
  coursesSnapshot.forEach(doc => { const data = doc.data(); if (data.name) newCache.classes.add(data.name); });

  const subjectsSnapshot = await db.collection('subjects').get();
  subjectsSnapshot.forEach(doc => {
    const data = doc.data();
    if (data.class && data.subject) {
      if (!newCache.subjectsByClass.has(data.class)) newCache.subjectsByClass.set(data.class, new Set());
  ...
  chaptersSnapshot.forEach(doc => {
    ...
    if (data.class && data.subject && data.chapter) {
      ...
      if (!newCache.chaptersByClassSubject.has(key)) newCache.chaptersByClassSubject.set(key, new Set());
...
function updateCacheForNewMapping(className, subject, chapter) {
  ...
  if (!metadataCache.subjectsByClass.has(className)) metadataCache.subjectsByClass.set(className, new Set());
  metadataCache.subjectsByClass.get(className).add(subject);
  const key = `${className}|${subject}`;
  if (!metadataCache.chaptersByClassSubject.has(key)) metadataCache.chaptersByClassSubject.set(key, new Set());
...
async function commitMixedWrites(createWrites, updateWrites) {
  ...
  for (let i = 0; i < allOperations.length; i += chunkSize) {
    ...
    for (const op of chunk) {
      if (op.type === 'set') batch.set(op.docRef, op.data);
      else if (op.type === 'update') batch.update(op.docRef, op.data);
    }
    await batch.commit();
  }
}
  ...
  try {
    ...
    batch.forEach(item => {
      if (!item.sent) {
        ...
        item.res.json({ status: 'error', message: 'An internal error occurred.', questionsProcessed: 0 });
      }
    });
  } finally {
    activeBatches--;
...
async function processBatch(batch) {
  ...
  for (const item of batch) {
    ...
    const validQuestions = questions.filter(q => q.questionText && Array.isArray(q.options) && q.options.length === 4 && typeof q.correctIndex === 'number' && q.solutionText);
  ...
  if (allQuestions.length === 0) {
    for (const item of batch) {
      if (!item.sent) {
        item.sent = true;
        clearRequestTimeout(item);
        item.res.json({ status: 'success', message: 'No valid questions provided.', questionsProcessed: 0 });
      }
    }
    return;
  }
  ...
  for (const { item, question, hash } of allQuestions) {
    const mapping = { class: item.class, subject: item.subject, chapter: item.chapter };
    ...
    if (!currentMappings) {
      currentMappings = [{ class: docData.class, subject: docData.subject, chapter: docData.chapter }];
    }
    const mappingExists = currentMappings.some(m => m.class === mapping.class && m.subject === mapping.subject && m.chapter === mapping.chapter);
    if (!mappingExists) {
      const path = docRef.path;
      if (!updatesMap.has(path)) {
        updatesMap.set(path, { docRef, currentMappings, newMappingsSet: new Set() });
      }
  ...
  for (const item of batch) {
    if (!item.sent) {
      ...
      const message = totalProcessed === 0 ? 'No new questions or mappings added.' : `Processed ${totalProcessed} questions (${counts.newCount} new, ${counts.updatedCount} updated).`;
      item.sent = true;
      clearRequestTimeout(item);
      item.res.json({ status: 'success', message, questionsProcessed: totalProcessed });
    }
  }
}
...
const authenticate = async (req, res, next) => {
  ...
  try {
    ...
    req.user = { userId: decoded.userId, ...userData };
    next();
  } catch (err) {
...
const checkPermission = (className) => async (req, res, next) => {
  ...
  try {
    const permissionSnapshot = await db.collection('permissions')
      .where('userId', '==', req.user.userId)
      .where('permittedClass', '==', className)
      .limit(1)
      .get();
    if (permissionSnapshot.empty) {
      return res.status(403).json({ success: false, error: `No permission to access class "${className}"` });
    }
    const permission = permissionSnapshot.docs[0].data();
    const now = admin.firestore.Timestamp.now();
    if (permission.expiredAt && permission.expiredAt.toDate() < now.toDate()) {
      return res.status(403).json({ success: false, error: `Permission for class "${className}" has expired` });
    }
    next();
  } catch (err) {
...
app.get('/api/quiz', authenticate, async (req, res, next) => {
  ...
  checkPermission(className)(req, res, async () => {
    try {
      ...
      if (!subject || !chapter) {
        return res.status(400).json({ success: false, error: 'Missing required query parameters: subject, chapter' });
      }
      let limit = 10;
      if (limitParam) {
      ...
      if (cursor) {
        ...
        if (isNaN(cursorNum)) {
          return res.status(400).json({ success: false, error: 'cursor must be a number' });
        }
...
function extractMappingsFromDoc(docData) {
  if (docData.mappings && Array.isArray(docData.mappings)) {
    ...
    return [{ class: docData.class, subject: docData.subject, chapter: docData.chapter }];
  }
  return [];
}
...
app.delete('/api/question', async (req, res) => {
  try {
    ...
    if (!questionId || typeof questionId !== 'string') {
      return res.status(400).json({ success: false, error: 'questionId is required and must be a string' });
    }
    if (!db) {
      console.error('Firestore not initialized');
      return res.status(500).json({ success: false, error: 'Database service unavailable' });
  ...
  try {
    ...
    if (questionIds && Array.isArray(questionIds) && questionIds.length > 0) {
      for (const id of questionIds) {
        if (typeof id !== 'string') {
          return res.status(400).json({ success: false, error: 'Each questionId must be a string' });
        }
      ...
      return res.status(400).json({ success: false, error: 'Invalid request. Provide either "questionIds" array or "class", "subject", "chapter"' });
    }
  ...
  try {
    ...
    if (mappings !== undefined && Array.isArray(mappings)) {
      ...
      if (currentData.mappings && Array.isArray(currentData.mappings)) {
        ...
        existingMappings = [{ class: currentData.class, subject: currentData.subject, chapter: currentData.chapter }];
      }
